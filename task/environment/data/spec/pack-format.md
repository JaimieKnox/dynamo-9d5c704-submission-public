...
(default 0), optional `segment_lambdas` (per-segment lambda overrides; when absent every index
uses `lambda`). Legacy `bootstrap_value` may appear and must be ignored.
