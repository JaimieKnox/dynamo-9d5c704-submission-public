...
def compute_gae(
    ...
    segment_lambdas=None, scale_through=None, segment_gammas=None,
):
    T = len(rewards)
    ...
    for t in reversed(range(T)):
        ...
        if scale_through[t]:
            v_next = v_next * scale
        gamma_t = float(gamma)
        if segment_gammas is not None:
            seg = segments[t]
            if seg < len(segment_gammas):
                gamma_t = float(segment_gammas[seg])
        delta = r + gamma_t * v_next * next_nonterminal[t] - values[t]
