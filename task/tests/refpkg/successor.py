...
def resolve_successor(
    ...
    if truncated[t] and t + 1 < horizon and segments[t] == segments[t + 1]:
        ...
        if src_t < open_t:
            src_t = open_t
        return float(src[src_t]), 1.0, True  # scale-through
    if t + 1 >= horizon or segments[t] != segments[t + 1]:
        if seam_boots is not None:
            return float(seam_boots[segments[t]]), 1.0, True  # scale-through
        return float(bootstrap_value), 1.0, True
    return float(boot_values[t + 1]), 1.0, False
